...
class BSPSpeechParser:
    def __init__(self):
        ...
        self.output_folder = os.path.join("..", "00 Original Dataset", "speech dataset", "bsp_speeches")
        ...
        if not os.path.exists(self.output_folder):
            ...
            os.makedirs(os.path.join(self.output_folder, "raw"), exist_ok=True)
            os.makedirs(os.path.join(self.output_folder, "csv"), exist_ok=True)

    def parse_date(self, date_str, is_end_date=False):
        ...
        try:
            ...
            if "/" in date_str and len(date_str.split("/")) == 2:
                ...
                if is_end_date:
                    # Create a datetime at the end of the day Philippine Time
                    ph_date = datetime(current_year, int(month), int(day), 23, 59, 59)
                else:
            ...
            # Determine the default datetime based on whether it's a start or end date
            now = datetime.now()
            if is_end_date:
                # Default to end of the year/month/day
                default_date = datetime(now.year, 12, 31, 23, 59, 59)
            else:
                # Default to start of the year/month/day
                default_date = datetime(now.year, 1, 1, 0, 0, 0)

            # For other formats, parse the date with the specific default
    ...
    def fetch_speeches(self, start_date=None, end_date=None):
        ...
        if end_date is None:
            ...
            end_date = self.parse_date(end_date, is_end_date=True)

        # Use a very old date if start_date is not provided to catch everything
        if start_date is None:
            start_date = "1990-01-01T00:00:00.000Z"
        else:
            start_date = self.parse_date(start_date, is_end_date=False)
    ...
    def clean_html_content(self, html_content):
        """Clean HTML content from the speech transcription using BeautifulSoup"""
        if not html_content:
            return ""
        ...
        # Use BeautifulSoup to safely extract raw text from HTML structures
        soup = BeautifulSoup(fixed_html, "html.parser")
        
        # Handle common block elements by ensuring they have a space after them
        # to prevent merging words from different paragraphs/lines.
        for tag in soup.find_all(["p", "br", "div", "li", "h1", "h2", "h3", "h4", "h5", "h6", "tr", "td"]):
            tag.append(" ")
            
        # .get_text() without a separator preserves inline tag connectivity (e.g., positio<strong>n</strong>)
        clean_text = soup.get_text()

        # Normalize whitespace: replaces tabs, newlines, and multiple spaces with a single space
    ...
    def extract_speech_data(self, speeches_json):
        ...
        for speech in speeches_json:
            ...
            # Get and clean all text fields
            title = self.clean_html_content(speech.get("Title", ""))
            place = self.clean_html_content(speech.get("Place", ""))
            occasion = self.clean_html_content(speech.get("Occasion", ""))
            speaker = self.clean_html_content(speech.get("Speaker", ""))
    ...
    def process_speeches(self, start_date=None, end_date=None):
        ...
        filename_prefix = f"bsp_speeches_{date_str_start}_to_{date_str_end}"
